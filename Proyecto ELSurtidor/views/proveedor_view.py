...
def ventana_proveedores(master):
    ...
    def cargar_datos():
        ...
        # iid almacena el id_proveedor de forma oculta en el Treeview (no visible al usuario)
        # values=fila[1:] muestra solo nombre, teléfono, dirección, email
    ...
    def nuevo():
        limpiar()  # limpia el formulario para ingresar un nuevo proveedor

    def guardar():
        if guardar_proveedor(entry_nombre.get(), entry_telefono.get(), entry_direccion.get(), entry_email.get()):
            limpiar()
            cargar_datos()
    ...
    def editar():
        ...
        confirmar = messagebox.askyesno("Confirmar", "¿Está seguro de modificar este proveedor?")
        if confirmar:
            if editar_proveedor(id_seleccionado["valor"], entry_nombre.get(), entry_telefono.get(), entry_direccion.get(), entry_email.get()):
                limpiar()
                cargar_datos()
    ...
    def eliminar():
        ...
        if confirmar:
            if borrar_proveedor(id_seleccionado["valor"]):
                limpiar()
                cargar_datos()
